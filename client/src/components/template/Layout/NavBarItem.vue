...
const itemLabel = computed(() =>
  props.item.isCurrentUser ? useAuthStore().tokenData?.username : props.item.label,
);
...
const menuClick = (event) => {
  ...
  if (props.item.lang) {
    useDarkModeStore().setLocale(props.item.lang);
  }
};
    ...
    >
      ...
      <span
        ...
        :class="{
          ...
          'text-blue-700': item.lang && $i18n.locale == item.lang,
        }"
        >{{ itemLabel }}</span
      >
      <BaseIcon
